...
/* ─────────────────────────────────────────────────────────
   Types — shared with the server wrapper (Hero.tsx) so the
   Sanity payload and the props this component receives are
   guaranteed to match.
   ───────────────────────────────────────────────────────── */
...
export interface HeroSlot {
  /** "large" | "small" — but kept as `string` so a typo in the CMS doesn't crash the page */
  size: string;
  pool: HeroFounder[];
}
...
/* ─────────────────────────────────────────────────────────
   Fallback defaults — used when Sanity returns null or a field
   is missing. Mirrors the original hardcoded values so the page
   NEVER goes blank if the CMS is unreachable.
   ───────────────────────────────────────────────────────── */
const FALLBACK_FOUNDER_SLOTS: HeroSlot[] = [
...
const FALLBACK_TITLE_L1 = "300+ Bets.";
const FALLBACK_TITLE_L2_BEFORE = "All On";
const FALLBACK_TITLE_L2_EMPHASIS = "Founders";
...
const FALLBACK_PRIMARY_CTA = "Get Investment";
const FALLBACK_SECONDARY_CTA = "View Portfolio";

/* ─────────────────────────────────────────────────────────
   Image URL helper — appends Sanity CDN transform params so
   the browser gets an optimised render (WebP/AVIF, resized).
   Local `/images/...` URLs pass through unchanged so the
   fallback path keeps working exactly as before.
   ───────────────────────────────────────────────────────── */
...
export default function HeroClient({ data }: { data?: HeroData | null }) {
  /* ── Resolve data: prefer CMS, fall back to defaults per-field ──
     Per-field fallbacks (instead of all-or-nothing) so a partially-edited
     Sanity document still renders cleanly. */
  const titleLine1 = data?.titleLine1 || FALLBACK_TITLE_L1;
  const titleLine2Before = data?.titleLine2Before || FALLBACK_TITLE_L2_BEFORE;
  const titleLine2Emphasis = data?.titleLine2Emphasis || FALLBACK_TITLE_L2_EMPHASIS;
  const subtitle = data?.subtitle || FALLBACK_SUBTITLE;
  const primaryCtaLabel = data?.primaryCtaLabel || FALLBACK_PRIMARY_CTA;
  const secondaryCtaLabel = data?.secondaryCtaLabel || FALLBACK_SECONDARY_CTA;
  // Founder slots: prefer Sanity, but graceful fallback per-slot so partial
  // content works while you're populating Studio.
  //   1. If Sanity has 5+ slots → use first 5 from Sanity
  //   2. If Sanity has 1-4 slots → take those, fill remaining slots from fallback
  //   3. If Sanity has 0 / null → all fallback
  // Within a slot, if a founder has no image (asset wasn't attached), swap
  // in the matching hardcoded founder from the same position so the card
  // never renders broken.
  const founderSlots: HeroSlot[] = (() => {
    const cmsSlots = data?.founderSlots ?? [];
    return Array.from({ length: 5 }, (_, i) => {
      const cmsSlot = cmsSlots[i];
      const fallback = FALLBACK_FOUNDER_SLOTS[i];
      if (!cmsSlot) return fallback;
      const repairedPool: HeroFounder[] = cmsSlot.pool.map((founder, j) => {
        if (!founder.image) {
          // No asset attached — borrow the matching position from fallback.
          return fallback.pool[j] ?? fallback.pool[0];
        }
        return founder;
      });
      return {
        size: cmsSlot.size || fallback.size,
        pool: repairedPool.length > 0 ? repairedPool : fallback.pool,
      };
    });
  })();
                ...
                <div key={i} className="shrink-0" style={{ perspective: "1200px" }}>
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      >
                        <Image
                          src={heroImageSrc(currentFounder.image, 600)}
